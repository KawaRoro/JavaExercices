		  ...
		  Scanner sc = new Scanner(System.in);
			...
			for(int i = 0; i < (tabTrad.length-1) ; i++ ) {
				for(int j = 0; j < (tabTrad[i].length-1) ; j++ ) {
					...
					if(stringTemp.equalsIgnoreCase(mot)) { // .toUpperCase()
						id_mot = j;
			...
			for(int i = 0; i < tabTrad.length ; i++ ) {
				if(id_langue != i) {
					System.out.println("La traduction de " +mot+ " est la suivante : " + tabTrad[i][id_mot] + " en " + tabLanguage[i]);
				}
			}
